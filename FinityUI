...
function finity.new(isdark, gprojectName, thinProject)
	...
	self2.ChangeToggleKey = function(NewKey)
		...
		finityData.UpConnection = game:GetService("UserInputService").InputEnded:Connect(function(Input)
	...
	function self2:Category(name)
		...
		category.button.MouseEnter:Connect(function()
			game:GetService("TweenService"):Create(category.button, TweenInfo.new(0.2), {BackgroundTransparency = 0.5}):Play()
		end)
		...
		function category:Sector(name)
			...
			function sector:Cheat(kind, name, callback, data)
                    ...
                    elseif string.lower(kind) == "color" or string.lower(kind) == "colorpicker" then
                        ...
                        cheat.hsvbar.MouseButton1Down:Connect(function()
                            ...
                            while uis:IsMouseButtonPressed'MouseButton1' do
                                ...
                                game:GetService("TweenService"):Create(cheat.arrowpreview, TweenInfo.new(0.1), {Position = UDim2.new(position, 0, 0.5, -6)}):Play()
					...
					elseif string.lower(kind) == "dropdown" then
						...
						function cheat.fadelist()
							...
							if cheat.dropped then
								...
								game:GetService("TweenService"):Create(cheat.list, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, math.clamp(cheat.list["UIListLayout"].AbsoluteContentSize.Y, 0, 150)), Position = UDim2.new(0, 0, 1, 0), ScrollBarImageTransparency = 0, BackgroundTransparency = 0.5}):Play()
							else
								for _, button in next, cheat.list:GetChildren() do
									if button:IsA("TextButton") then
						...
						function cheat:RefreshOptions(newoptions)
							options = newoptions
							cheat.value = data.options[1]
							cheat.selected.Text = data.options[1]
                ...
                function cheat:SetLabelText(Text)
